...
ArduinoManager.prototype.uploadSketch = function (board, callback) {
    ...
    if (board === 'neo') {
        ...
        this.__execUpload(false, 'udooneo-m4uploader', [__dirname+'/sketches/neo.fw'], callback);
    }else if(board === 'quad'){
        //bossac-udoo --port=ttymxc3 -U false -e -w -v -b quad.fw -R
        this.__execUpload(false, 'bossac-udoo', ['--port=ttymxc3', '-U' ,'false', '-e', '-w', '-v', '-b', __dirname+'/sketches/quad.fw', '-R'], callback);
    }else if(board === 'x86'){
        const port = `/dev/${this.port}`;
        this.resetArduino101(port,  (err) => {
            if(err) callback("Reset port error");
            else{
                this.__execUpload(true, 'arduino101load',
                    ['-dfu=/usr/bin',
...
ArduinoManager.prototype.__execUpload = function(resetPort, command, args, callback) {
...
ArduinoManager.prototype.resetArduino101 = function (port, callbaclk) {
    ...
    sp.on("error", function () {
        ...
        callbaclk("Error reset port", null);
    });
};
