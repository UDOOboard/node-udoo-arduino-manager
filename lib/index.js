...
ArduinoManager.prototype.uploadSketch = function (board, callback) {
    if (board === 'neo') {
        ...
        execUpload('udooneo-m4uploader lib/sketchs/neo.fw', callback);
    }else if(board === 'quad'){
        //bossac-udoo --port=ttymxc3 -U false -e -w -v -b quad.fw -R
        execUpload('bossac-udoo --port=ttymxc3 -U false -e -w -v -b lib/sketchs/quad.fw -R', callback);
    }else if(board === 'x86'){
        //TODO
...
function execUpload(command, callback) {
    ...
    exec(command, function (err, stdout, stderr) {
        ...
        if (stderr) if (callback) callback(stderr);
        else if (callback) callback(null, 'success');
    });
}
