...
ArduinoManager.prototype.version = function(pin, mode, callback){
    ...
    this.__getBelzedooPort(function (err) {
        if (err) callback(err);
            ...
            that.method.hiBelzedoo(that.belzedoo, callback);
        }
    });
};
...
ArduinoManager.prototype.setPinMode = function(pin, mode, callback){
    ...
    this.__getBelzedooPort(function (err) {
        if (err){
            ...
            that.method.writePinMode(that.belzedoo, data);
        }
    });
};
...
ArduinoManager.prototype.digitalWrite = function(pin, value, callback){
    ...
    this.__getBelzedooPort(function (err) {
        if (err) callback(err);
            ...
            that.method.writeDigital(that.belzedoo, data);
        }
    });
};
...
ArduinoManager.prototype.attachInterrupt = function(pin, mode, callback){
    ...
    this.__getBelzedooPort(function (err) {
        if (err) callback(err);
            ...
            this.method.attachInterrupt(this.belzedoo, data);
            this.belzedoo.read(data.id, function (err) {
                if(!err)callback();
            });
        }
    });
...
ArduinoManager.prototype.detachInterrupt = function(pin, callback){
    ...
    this.__getBelzedooPort(function (err) {
        if (err) callback(err);
            ...
            this.method.detachInterrupt(this.belzedoo, data);
            this.belzedoo.read(data.id, function (err) {
                if(!err)callback();
            });
        }
